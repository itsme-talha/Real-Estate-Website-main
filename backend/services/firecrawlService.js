...
// Updated for Pakistani real estate platforms.
const SEARCH_SOURCES = {
    'zameen':      { domain: 'zameen.com',      limit: 6 },
    'graana':      { domain: 'graana.com',      limit: 6 },
    'olx':         { domain: 'olx.com.pk',      limit: 5 },
    'agency21':    { domain: 'agency21.com.pk', limit: 4 },
};
...
const SEARCH_RESULT_SCHEMA = {
    ...
    properties: {
        properties: {
            type: "array",
            description: "All FOR SALE property listings found on this page. Skip PG, hostels, and rentals.",
            items: {
                type: "object",
                properties: {
                    building_name:          { type: "string", description: "Society, phase or project name (e.g., DHA Phase 6, Bahria Town)" },
                    ...
                    bhk_config:             { type: "string", description: "e.g. 2 Bed, 3 Bed, 5 Marla, 1 Kanal" },
                    location_address:       { type: "string", description: "Full address with locality and city" },
                    total_price:            { type: "string", description: "Total purchase price e.g. PKR 1.65 Crore or 165 Lakh" },
                    price_per_sqft:         { type: "string", description: "Price per sq ft e.g. PKR 12,500/sqft" },
                    carpet_area_sqft:       { type: "string", description: "Covered area in Marla, Kanal or sqft" },
                    superbuiltup_area_sqft: { type: "string", description: "Total land area" },
                    ...
                    rera_number:            { type: "string", description: "Approval status (LDA/CDA/RDA approved), blank if absent" },
                    amenities:              { type: "array", items: { type: "string" }, description: "Top 5 amenities" },
                    nearby_landmarks:       { type: "array", items: { type: "string" }, description: "Nearby metrobus, school, hospital" },
...
const SEARCH_RESULT_PROMPT =
    ...
    "Each property must have a total purchase price in Crores or Lakhs (PKR) — NOT a rental price in /month. " +
    "Skip hostel, paying guest, and rental listings entirely. " +
 ...
 * e.g. "3 bed house for sale in DHA Phase 6 Lahore under 5 crore ready to move"
 */
function buildSearchQuery({ city, locality, bhk, maxPrice, propertyType, possession }) {
    const parts = [];

    if (bhk && bhk !== 'Any') {
        const bedTerm = bhk.replace(/BHK/i, 'Bed');
        parts.push(bedTerm);
    }

    const typeTerm = PROPERTY_TYPE_SEARCH_TERMS[propertyType] || 'house';
...
/**
 * Parse a Pakistani price string to a float in Crores.
 * Handles: "PKR 1.65 Cr", "45 L", "45 Lakh", raw numbers.
 * Returns null if the string looks like a rental price or can't be parsed.
 */
function parsePriceToCrores(priceStr) {
    if (!priceStr || typeof priceStr !== 'string') return null;
    const s = priceStr.replace(/[pkrrs₹,\s]/gi, '').toLowerCase();
    ...
    // Raw absolute number (e.g. 16500000) — assume PKR
 ...
 * slice contains a proportional mix (e.g. 6 zameen + 6 graana)
...
function filterValidProperties(properties, minPrice, maxPrice) {
    ...
    return properties.filter(p => {
        ...
        // Reject URLs that are clearly rentals/PG/Hostel
        const url = p.property_url || '';
        if (/paying.guest|pg-for-rent|for-rent|hostel/.test(url)) return false;
...
class FirecrawlService {
    ...
    async findProperties({
        ...
        try {
            ...
            const activeSources = ['zameen', 'graana', 'olx'];
            if (includeNoBroker) activeSources.push('agency21');
                ...
                return this.searchCircuit.execute(async () => {
                    const response = await withTimeout(
                        this.firecrawl.search(query, {
                            limit: srcLimit,
                            location: 'PK', // Pakistani location for accurate results
                            scrapeOptions: {
                                formats: [{
                                    type: 'json',
            ...
            if (smartMinPrice > 0) {
                console.log(`[DEBUG] Smart min price      : PKR ${smartMinPrice.toFixed(2)} Cr (${(smartMinPrice * 100).toFixed(0)} L floor for ${maxPriceNum} Cr budget)`);
            }
    ...
    async getLocationTrends(city, limit = 5) {
        try {
            ...
            const url = `https://www.zameen.com/Trends/${formattedLocation}`; // Updated to zameen trends URL structure
            ...
            const locationSchema = {
                ...
                properties: {
                    locations: {
                        type: "array",
                        description: `Price trend data for ${limit} major localities in ${city}`,
                        items: {
                            type: "object",
                            properties: {
            ...
            const scrapeResult = await this._scrapeWithRetry(url, {
                formats: [{
                    type: 'json',
                    prompt: `Extract price trend data for ${limit} major localities in ${city}. Include location name, price per sqft (PKR), yearly percent increase, and rental yield.`,
    ...
    async _scrapeWithRetry(url, baseOpts, label) {
        ...
        for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
            ...
            const opts   = useGeo ? { ...baseOpts, location: { country: "PK" } } : { ...baseOpts }; // Changed from IN to PK
