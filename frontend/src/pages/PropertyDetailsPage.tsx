...
const PropertyDetailsPage: React.FC = () => {
      ...
      <div className="bg-[#154D57] min-h-screen">
        ...
        <div className="flex items-center justify-center py-32">
          <div className="text-center">
            <span className="material-icons text-5xl text-[#B7A08B] mb-4">error_outline</span>
            <p className="font-manrope text-xl text-white/80 mb-6">{error || 'Property not found'}</p>
            <Link
              to="/properties"
              className="bg-[#B7A08B] text-[#154D57] font-manrope font-bold px-8 py-3 rounded-xl hover:bg-white transition-all inline-block shadow-lg"
            >
              Back to Properties
            </Link>
          </div>
  ...
  // Extract city from location string (e.g. "Gulberg, Lahore, Punjab" → "Lahore")
  // Pakistani addresses typically end with province, so use second-to-last part as city
  const cityParts = property.location.split(',').map(s => s.trim());
  const city = cityParts.length >= 3
    ? cityParts[cityParts.length - 2]       // "Area, City, Province" → City
    : cityParts.length === 2
      ? cityParts[0]                        // "City, Province" → City
  ...
  return (
    <div className="bg-[#154D57] min-h-screen">
      ...
      {/* Main Content Area */}
      <div className="bg-[#0F3B43] py-12 shadow-inner">
        <div className="max-w-[1280px] mx-auto px-8">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            {/* Left Column - Main Content */}
            <div className="lg:col-span-2">
              <div className="bg-[#12434D] border border-[#B7A08B]/20 rounded-2xl p-8 shadow-lg">
                {/* About Section */}
