...
export interface SearchParams {
  city: string;
  locality: string;        // specific area within city e.g. "DHA", "Gulberg"
...
const AIHubProductionPage: React.FC = () => {
  ...
  return (
    <div className="bg-[#154D57]">
      {/* Hero Section */}
      <section className="relative bg-[#0F3B43] overflow-hidden border-b border-[#B7A08B]/20">
        {/* Decorative blurs */}
        <div className="absolute top-20 right-20 w-80 h-80 bg-[#B7A08B]/10 rounded-full blur-[80px]" />
        <div className="absolute bottom-10 left-10 w-60 h-60 bg-[#B7A08B]/5 rounded-full blur-[60px]" />

        <div className="max-w-[1280px] mx-auto px-8 pt-32 pb-20 relative z-10">
          <div className="text-center max-w-3xl mx-auto">
            {/* Badge */}
            <div className="inline-flex items-center gap-3 bg-[#B7A08B]/10 border border-[#B7A08B]/30 rounded-full px-5 py-2.5 mb-8 shadow-sm">
              <span className="font-material-icons text-[#B7A08B] text-lg">smart_toy</span>
              <span className="font-manrope font-bold text-sm text-[#B7A08B] uppercase tracking-wider">
                AI-Powered Feature
              </span>
            </div>

            <h1 className="font-fraunces text-5xl lg:text-6xl font-bold text-white leading-tight mb-6 drop-shadow-sm">
              AI Property Hub
            </h1>
            <p className="font-manrope text-lg text-white/80 leading-relaxed mb-10 max-w-2xl mx-auto">
            ...
            <div className="flex flex-wrap justify-center gap-4 mb-12">
              <a
                ...
                className="bg-[#B7A08B] text-[#154D57] font-manrope font-bold text-lg px-8 py-4 rounded-xl shadow-lg hover:bg-white transition-all hover:shadow-xl inline-flex items-center gap-3"
              >
              ...
              <Link
                to="/properties"
                className="border-2 border-[#B7A08B]/50 text-[#B7A08B] font-manrope font-bold text-lg px-8 py-4 rounded-xl hover:border-[#B7A08B] hover:text-white hover:bg-[#B7A08B]/10 transition-all inline-flex items-center gap-2"
              >
                Browse Properties
            ...
            {/* Info Banner */}
            <div className="bg-[#B7A08B]/10 border border-[#B7A08B]/30 rounded-2xl p-6 max-w-xl mx-auto shadow-sm">
              <div className="flex items-start gap-3">
                <span className="font-material-icons text-[#B7A08B] text-xl mt-0.5">info</span>
                <p className="font-manrope text-sm text-white/80 text-left leading-relaxed">
      ...
      <section className="max-w-[1280px] mx-auto px-8 py-20">
        <div className="text-center mb-14">
          <h2 className="font-fraunces text-3xl lg:text-4xl font-bold text-white mb-4">
            What You'll Get Locally
          </h2>
          <p className="font-manrope text-white/70 max-w-xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {features.map((feature, index) => (
            <div
              key={index}
              className="bg-[#12434D] border border-[#B7A08B]/20 rounded-2xl p-6 hover:shadow-xl hover:border-[#B7A08B]/50 transition-all group"
            >
              <div className="w-12 h-12 bg-[#B7A08B]/10 rounded-xl flex items-center justify-center mb-4 group-hover:bg-[#B7A08B] transition-colors">
                <span className="font-material-icons text-[#B7A08B] group-hover:text-[#154D57] transition-colors">
                  {feature.icon}
                </span>
              </div>
              <h3 className="font-syne font-bold text-lg text-white mb-2">{feature.title}</h3>
              <p className="font-manrope text-sm text-white/70 leading-relaxed">{feature.description}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* Setup Steps */}
      <section className="bg-[#0F3B43] py-20 border-t border-[#B7A08B]/20 shadow-inner">
        <div className="max-w-[1280px] mx-auto px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {steps.map((item) => (
              <div key={item.step} className="bg-[#12434D] rounded-2xl p-6 border border-[#B7A08B]/30 shadow-md">
                <span className="font-space-mono text-4xl font-bold text-[#B7A08B]/40 block mb-3">
                  {item.step}
                </span>
                <h3 className="font-syne font-bold text-lg text-white mb-3">{item.title}</h3>
                <code className="block bg-[#0F3B43] text-[#B7A08B] border border-[#B7A08B]/20 font-space-mono text-xs rounded-lg p-3 overflow-x-auto">
                  {item.command}
                </code>
              </div>
            ))}
          ...
          <div className="text-center mt-12">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-3 bg-[#B7A08B] text-[#154D57] font-manrope font-bold px-8 py-4 rounded-xl hover:bg-white transition-all shadow-lg hover:shadow-xl"
            >
...
const AIPropertyHubPage: React.FC = () => {
  useSEO({
    ...
    description: 'AI-powered property search, market analysis, location trends, and investment insights for Pakistan real estate.',
  });

  /* ── AI Hub disabled → show "download & run locally" page ── */
  if (!AI_HUB_ENABLED) {
    return (
      <div className="bg-[#154D57] min-h-screen">
        <Navbar />
        <AIHubProductionPage />
        <Footer />
      </div>
...
const AIHubDevPage: React.FC = () => {
  ...
  const handleSearch = async (params: SearchParams) => {
    ...
    try {
      const maxPriceInRupees = params.maxBudget * 10_000_000; // Cr → PKR
  ...
  return (
    <div className="bg-[#154D57] min-h-screen">
      ...
      {/* Hero — search form */}
      {AIHeroSection && (
        <React.Suspense fallback={<div className="h-96 flex items-center justify-center"><div className="w-12 h-12 border-4 border-[#B7A08B] border-t-transparent rounded-full animate-spin" /></div>}>
          <AIHeroSection
      ...
      <div ref={resultsRef}>
        {hasSearched && AISearchResults && AIAnalysisPanel && (
          <React.Suspense fallback={null}>
            <section className="bg-[#12434D] py-14 border-t border-[#B7A08B]/20">
      ...
      {hasSearched && !searchLoading && properties.length > 0 && AILocationTrends && (
        <React.Suspense fallback={null}>
          {!hasLoadedTrends ? (
            <section className="bg-[#0F3B43] py-10 border-t border-[#B7A08B]/20 shadow-inner">
              <div className="max-w-[1200px] mx-auto px-6 text-center">
                <p className="font-manrope text-sm text-white/70 mb-4">
                  Want to see price trends and rental yields for {searchParams.city}?
                </p>
                <button
                  onClick={() => fetchTrends(searchParams.city)}
                  className="inline-flex items-center gap-2 bg-[#B7A08B] hover:bg-white text-[#154D57] font-manrope font-bold text-sm px-6 py-3 rounded-xl transition-all shadow-lg"
                >
