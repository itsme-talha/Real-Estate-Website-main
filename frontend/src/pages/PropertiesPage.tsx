...
const PropertiesPage: React.FC = () => {
  ...
  const filteredProperties = useMemo(() => {
    ...
    // Slider uses 0–200 scale where each unit = 1,000,000 PKR (10 Lakhs)
      ...
      <div className="flex">
        ...
        <div className="flex-1">
          ...
          {error && !loading && (
            <div className="flex items-center justify-center py-24">
              <div className="text-center">
                <span className="material-icons text-4xl text-[#B7A08B] mb-4">error_outline</span>
                <p className="font-manrope text-white/80 mb-4">{error}</p>
                <button
                  onClick={() => window.location.reload()}
                  className="bg-[#B7A08B] text-[#154D57] font-manrope font-bold px-6 py-2 rounded-lg hover:bg-white transition-all shadow-md"
                >
                  Retry
                </button>
              </div>
          ...
          {!loading && !error && filteredProperties.length === 0 && (
            <div className="flex items-center justify-center py-24">
              <div className="text-center">
                <span className="material-icons text-4xl text-[#B7A08B]/50 mb-4">search_off</span>
                <p className="font-manrope text-white mb-2 text-lg">No properties found</p>
                <p className="font-manrope font-extralight text-sm text-white/60">Try adjusting your filters</p>
              </div>
            </div>
          )}
