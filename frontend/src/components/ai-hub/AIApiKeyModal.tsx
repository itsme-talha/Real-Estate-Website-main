...
const AIApiKeyModal: React.FC<AIApiKeyModalProps> = ({ isOpen, onClose, onKeysChanged }) => {
  ...
  return (
    <div className="fixed inset-0 z-[999] flex items-center justify-center p-4">
      {/* Backdrop */}
      <div className="absolute inset-0 bg-[#0F3B43]/80 backdrop-blur-sm" onClick={onClose} />

      {/* Panel */}
      <div className="relative w-full max-w-lg bg-[#154D57] border border-[#B7A08B]/30 rounded-2xl shadow-2xl overflow-hidden">

        {/* Header */}
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-[#B7A08B]/20">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 bg-[#B7A08B]/20 rounded-xl flex items-center justify-center border border-[#B7A08B]/30">
              <Key className="w-4 h-4 text-[#B7A08B]" />
            </div>
            <div>
              <h2 className="font-syne font-bold text-white text-lg">Your API Keys</h2>
              <p className="font-manrope text-xs text-white/50">Saved in your browser only — never sent to our servers.</p>
            </div>
          </div>
          <button onClick={onClose} className="text-white/40 hover:text-[#B7A08B] transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        {/* Note */}
        <div className="mx-6 mb-4 flex items-start gap-2 bg-[#0F3B43] border border-[#B7A08B]/20 rounded-xl px-4 py-3">
          <AlertCircle className="w-4 h-4 text-[#B7A08B] shrink-0 mt-0.5" />
          <p className="font-manrope text-xs text-white/60 leading-relaxed">
            Your keys are stored only in <strong className="text-white/90">this browser</strong>. They are sent directly to our backend with each request and used only to call the AI services on your behalf.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3 px-6 pb-6">
          <button
            ...
            className="flex-1 flex items-center justify-center gap-2 bg-[#B7A08B] hover:bg-white disabled:opacity-40 disabled:cursor-not-allowed text-[#154D57] font-manrope font-bold text-sm py-3 rounded-xl transition-all"
          >
          ...
          {(hasGithub || hasFirecrawl) && (
            <button
              onClick={handleClear}
              className="flex items-center gap-2 bg-red-500/10 hover:bg-red-500/20 border border-red-500/30 text-red-400 font-manrope font-bold text-sm py-3 px-5 rounded-xl transition-all"
            >
...
const StatusBadge: React.FC<{ label: string; active: boolean }> = ({ label, active }) => (
  <div className={`flex items-center gap-2 px-3 py-2 rounded-lg border ${active ? 'bg-emerald-500/20 border-emerald-500/40' : 'bg-[#0F3B43] border-[#B7A08B]/20'
    }`}>
    {active
      ? <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
      : <AlertCircle className="w-3.5 h-3.5 text-[#B7A08B]/50 shrink-0" />}
    <span className={`font-manrope text-xs ${active ? 'text-emerald-300' : 'text-white/50'}`}>{label}</span>
    <span className={`ml-auto font-space-mono text-[10px] ${active ? 'text-emerald-400' : 'text-[#B7A08B]/50'}`}>
      {active ? '✓ set' : 'not set'}
    </span>
  </div>
);
...
const KeyInput: React.FC<KeyInputProps> = ({
  ...
  <div>
    <div className="flex items-center justify-between mb-1.5">
      <label className="font-space-mono text-[10px] text-[#B7A08B] uppercase tracking-widest">{label}</label>
      <a href={linkHref} target="_blank" rel="noopener noreferrer"
        className="flex items-center gap-1 font-manrope text-[11px] text-[#B7A08B] hover:text-white transition-colors">
        {linkText} <ExternalLink className="w-3 h-3" />
      </a>
    </div>
    <div className="relative bg-[#0F3B43] border border-[#B7A08B]/30 rounded-xl px-4 py-3 flex items-center gap-3 focus-within:border-[#B7A08B] transition-all">
      <Key className="w-4 h-4 text-[#B7A08B]/60 shrink-0" />
      <input
        type={show ? 'text' : 'password'}
        value={value}
        onChange={e => onChange(e.target.value)}
        placeholder={placeholder}
        className="flex-1 bg-transparent font-space-mono text-xs text-white outline-none placeholder:text-white/30"
        autoComplete="off"
      />
      <button type="button" onClick={onToggleShow} className="text-[#B7A08B]/60 hover:text-[#B7A08B] transition-colors">
