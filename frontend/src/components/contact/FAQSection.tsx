...
const FAQSection: React.FC = () => {
  ...
  const faqs: FAQItem[] = [
    ...
    },
    {
      ...
      answer: "We currently cover major metropolitan areas including Lahore, Karachi, Islamabad, Rawalpindi, and Faisalabad. We're expanding to more cities across Pakistan and will update our coverage area regularly."
    },
    {
      id: 3,
      ...
      <div className="max-w-[1280px] mx-auto px-8">
        ...
        <div className="text-center mb-16">
          <div className="flex justify-center mb-3">
            <span className="font-space-mono text-xs text-[#B7A08B] uppercase tracking-widest">
              Help Center
            </span>
          </div>
          ...
          <p className="font-manrope text-lg text-white/80 leading-relaxed max-w-[640px] mx-auto">
        ...
        <div className="max-w-[800px] mx-auto space-y-4">
          {faqs.map((faq, index) => (
            <div 
              key={faq.id}
              className="bg-[#12434D] border border-[#B7A08B]/20 rounded-xl overflow-hidden transition-all"
            >
              {/* Question */}
              <button
                onClick={() => toggleFAQ(index)}
                className="w-full flex items-center gap-4 p-6 text-left hover:bg-[#0F3B43] transition-colors"
              >
                {/* Number Badge */}
                <div className="w-8 h-8 bg-[#154D57] border border-[#B7A08B]/30 rounded-full flex items-center justify-center flex-shrink-0">
                  <span className="font-syne font-bold text-sm text-[#B7A08B]">
                    {String(faq.id).padStart(2, '0')}
                  </span>
                </div>

                {/* Question Text */}
                <h3 className="flex-1 font-syne font-bold text-lg text-white">
                  {faq.question}
                </h3>

                {/* Expand/Collapse Icon */}
                <span className={`material-icons text-[#B7A08B] transition-transform ${
        ...
        <div className="text-center mt-12">
          <a 
            href="#" 
            className="inline-flex items-center gap-2 font-manrope font-bold text-base text-[#B7A08B] hover:text-white transition-colors group"
          >
